function(test)
  ...
  # cmake_script_test(--async-wait --process-handle) ans(res) assertf({res.pid}
  # MATCHES "[1-9][0-9]*")
  ...
  # assert that --exit-code  returns the correct return code and working
  ...
  # assert that --exit-code returns a non 0 return code whne execution reports
  # error
  ...
  ans(res)
  assert(DEREF "{res.exit_code}" STREQUAL "0") # return code should be error
                                               # free
  assert(DEREF "{res.stdout}" MATCHES "hello") # stdout should contain only
                                               # hello (possibly a line break)
  assert(DEREF "{res.start_info.timeout}" STREQUAL "-1") # timeout default value
                                                         # should be -1 (no
                                                         # timeout)
  ...
  assert(DEREF "{res.stderr}" MATCHES "byebye") # stdout should contain only
                                                # hello (possibly a line break)
  assert(DEREF "{res.start_info.timeout}" STREQUAL "-1") # timeout default value
                                                         # should be -1 (no
                                                         # timeout)
