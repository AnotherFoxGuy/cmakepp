function(test)
  ...
  # the workind directory needs to exist. maybe this needs to be ensured within
  # execute.
  mkdir(mytempdir)

  # check if command is actually started in working directory in sync mode
  execute_script("echo_append(\${CMAKE_CURRENT_BINARY_DIR})" WORKING_DIRECTORY
                 "mytempdir") # delegates to cmake
  ans(res)
  assert("${res}" MATCHES "/mytempdir$")

  # check if command is actually started in working directory in async mode
  execute_script("echo_append(\${CMAKE_CURRENT_BINARY_DIR})" WORKING_DIRECTORY
                 "mytempdir" --async)
  ans(process_handle)
  ...
  # act
  execute(${CMAKE_COMMAND} -P errorscript.cmake WORKING_DIRECTORY dir1
          --process-handle)
  ans(res)
  ...
  execute(
    ...
    "hello;hello"
    --success-callback
    "[](handle)map_append(${context} result 'success{{exit_code}}')"
    --error-callback
    "[](handle) map_append(${context} result 'error{{exit_code}}')"
    --silent-fail)
  ...
  execute(
    ...
    cmake
    -E
    echo_appendFAILBECAUSESTUPIDERROR
    "hello;hello"
    --success-callback
    "[](handle)map_append(${context} result 'success{{exit_code}}')"
    --error-callback
