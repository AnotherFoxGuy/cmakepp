function(parse_sequence rstring)
  ...
  function(eval_sequence_expression rstring key res_map expression set_map)
    ...
    if("${expression}" MATCHES "[^@]*\\|")
      ...
      eval_sequence_expression("${rstring}" "${key}" "${res_map}" "${left}"
                               "${set_map}")
      ans(success)
      if(success)
        return(true)
      ...
      eval_sequence_expression("${rstring}" "${key}" "${res_map}" "${right}"
                               "${set_map}")
      return_ans()
    endif()
    ...
    if("${expression}" MATCHES "[a-zA-Z0-9_-]+\\?.+")
      ...
      if(has_value)
        ...
        return(false)
      elseif(DEFINED else)
        eval_sequence_expression("${rstring}" "${key}" "${res_map}" "${else}"
                                 "${set_map}")
        ans(success)
        if(success)
          return(true)
  ...
  foreach(sequence_key ${sequence_keys})
    ...
    eval_sequence_expression("${str}" "${sequence_key}" "${result_map}"
                             "${sequence_id}" "${set_map}")
    ans(success)
    if(NOT success)
      return()
...
# foreach(sequence_key ${sequence_keys})
#
# map_tryget("${sequence}" "${sequence_key}") ans(sequence_id)
#
# if("${sequence_id}" MATCHES "^@") string(SUBSTRING "${sequence_id}" 1 -1
# sequence_id) map_set("${result_map}" "${sequence_key}" "${sequence_id}")
#
# else() set(ignore false) set(optional false) if("${sequence_id}" MATCHES
# "^\\?") string(SUBSTRING "${sequence_id}" 1 -1 sequence_id) set(optional true)
# endif() if("${sequence_id}" MATCHES "^/") string(SUBSTRING "${sequence_id}" 1
# -1 sequence_id) set(ignore true) endif()
#
# parse_string("${str}" "${sequence_id}") ans(res)
#
# list(LENGTH res len)
#
# if(${len} EQUAL 0 AND NOT optional) return() endif()
#
# if(NOT "${ignore}") map_set("${result_map}" "${sequence_key}" "${res}")
