# `(<process start info> [--process-handle] [--exit-code] [--async]
# [--silent-fail] [--success-callback <callable>]  [--error-callback <callable>]
# [--state-changed-callback <callable>])-><process handle>|<exit
# code>|<stdout>|<null>`
#
# *options* * `--process-handle` * `--exit-code` * `--async` * `--silent-fail` *
...
function(execute)
  ...
  ans(async)
  # list_extract_flag(args --async-wait) ans(wait) if(wait) set(async true)
  # endif()
  ...
  # debug here print_vars(start_info.command start_info.command_arguments)
  ...
  if(async)
    ...
    ans(pid)
    if(NOT pid)
      message(FATAL_ERROR FORMAT
                          "could not find command '{start_info.command}'")
    endif()

    if(exit_code AND silent_fail)
      error(
        "process {start_info.command} failed with {process_handle.exit_code}")
      return()
    endif()
