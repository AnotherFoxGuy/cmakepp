...
function(process_start_Linux process_handle)
    ...
    # create a temporary shell script which starts bash with the specified command output of the command is stored in stdout file error of the command is stored in stderr file
    # return_code is stored in return_code file and the created process id is stored in pid_out
    shell_tmp_script("( bash -c \"(cd ${working_directory}; ${command_string} > ${stdout} 2> ${stderr})\" ; echo $? > ${return_code}) & echo $! > ${pid_out}")
    ans(script)
    # execute the script in bash with nohup which causes the script to run detached from process
