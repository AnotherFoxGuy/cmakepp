# git_cached_clone(<remote uri:<~uri>> <?target_dir> [--readonly] ([--file
# <>]|[--read<>]) [--ref <git ref>])->
function(git_cached_clone remote_uri)
  ...
  if(read OR file)
    ...
    git_lean(clone --reference "${repo_cache_dir}" "${remote_uri}"
             "${target_dir}")
    ans_extract(error)
    if(error)
      message(
        FATAL_ERROR
          "failed to reference clone params:(clone --reference \"${repo_cache_dir}\" \"${remote_uri}\" \"${target_dir}\") "
      )
    endif()
    pushd("${target_dir}")
