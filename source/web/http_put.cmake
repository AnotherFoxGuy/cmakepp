# http_put() ->
#
# flags: --response                                              flag will
# return the http response object
#
# --json flag will deserialize the result data
#
# --exit-code                                             flag will return the
# http clients return code non-zero indicates an error --show-progress flag
# causes a console message which indicates the progress of the operation
#
# --raw                                                           flag witll
# cause input to be sent raw if flag is not specified the input is serialized to
# json before sending
#
# --file <file>                                                   flag PUT the
# specified file instead of the input
#
# --silent-fail                                                   flag causes
# function to return nothing if it fails (only usable if --response was not set)
#
# --timeout <n>                                                   value
#
# --inactivity-timeout <n>        value
#
# events: on_http_put(<uri> <content>)-> <uri?>: event is called before put
# request is performed user may cancel event and return a modified uri which is
...
function(http_put uri)
  ...
  file(
    UPLOAD "${content_file}" "${uri}"
    STATUS client_result
    LOG http_log
    ${show_progress} ${timeout} ${inactivity_timeout})
  ...
  if(NOT return_response AND client_status)
    ...
    if(NOT silent_fail)
      message(
        FATAL_ERROR "http_put failed: ${client_message} - ${client_status}")
    endif()
    return()
  endif()
