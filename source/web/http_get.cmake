# http_get(<~uri> <?content:<structured data>> [--progress] [--response]
# [--exit-code] )-> <http response>
#
# flags: --json         flag deserializes the content and returns it
# --show-progress     flag prints the progress of the download to the console
# --response     flag --exit-code  flag --silent-fail  flag
#
function(http_get uri)
  ...
  # actual request - uses file DOWNLOAD which uses cUrl internally
  file(
    DOWNLOAD "${uri}" "${target_path}"
    STATUS status
    ${log} ${show_progress}
    TLS_VERIFY OFF
    ${args})
  ...
  # read content if client was executed correctly afterwards delete file
  if(NOT client_status)
    ...
    if(NOT silent_fail AND NOT return_response)
      rm("${target_path}")
      if("$ENV{TRAVIS}")
        # do not show the query if travis build because it could contain
        # sensitive data
  ...
  # if the response is not to be returnd check if deserialization is wished and
  # and return content
