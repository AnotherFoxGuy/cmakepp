...
function(continuation_resolve promise continuation)
  ...
  if(continuation_task)
    # task_anonymous("" () #message("resolving ${promise}'s task continuation
    # ${continuation} with ${value}") map_set(${continuation_task} arguments
    # ${value}) set(promise ${continuation}) task_invoke(${continuation_task})
    # ans(result) #message("task returned \${result}")
    ...
    # task_anonymous("" () #message("resolving ${promise}'s value continuation
    # ${continuation} with ${value}") promise_resolve("${continuation}"
    # "${value}") ) ans(task)
    task_from_cmake_code("promise_resolve(\"${continuation}\" \"${value}\")")
    ans(task)
  endif()

  # use the task_queue provided by the continuation if that fails use the task
  # queue from the previous promise if that failes use the global task queue
