...
function(continuation_resolve promise continuation)
    ...
    ans(value)
    # decide depending on value and continuation_task what to do.
    if(continuation_task)
        # task_anonymous("" () #message("resolving ${promise}'s task continuation ${continuation} with ${value}") map_set(${continuation_task} arguments ${value}) set(promise
        # ${continuation}) task_invoke(${continuation_task}) ans(result) #message("task returned \${result}") promise_resolve("${continuation}" "\${result}") )
        address_new()
        ans(value_adr)
      ...
      ")
        ...
        # task_anonymous("" () #message("resolving ${promise}'s value continuation ${continuation} with ${value}") promise_resolve("${continuation}" "${value}") ) ans(task)
        task_from_cmake_code("promise_resolve(\"${continuation}\" \"${value}\")")
        ans(task)
    endif()

    # use the task_queue provided by the continuation if that fails use the task queue from the previous promise if that failes use the global task queue
