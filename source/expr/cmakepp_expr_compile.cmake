...
# $[...] `
#
# compiles the specified cmakepp code to pure cmake code replacing `$[...]` with
# the result of the cmakepp expression syntax (see `expr(...)`) e.g.
# ~~~
# function(my_name)
#  return("tobi")
# endfunction()
...
function(cmakepp_expr_compile content)
  cmake_token_range(" ${content}") # prepend whitespace (because of replace
                                   # edgecase)
  ans(range)
  ...
  string_codes()
  set(regex
      "[\\$${bracket_open_code}${bracket_close_code}]|[^\\$${bracket_open_code}${bracket_close_code}]+"
  )
  ...
  foreach(invocation ${invocations})
    ...
    cmake_token_range_filter("${args_begin};${args_end}" NOT type MATCHES
                             "(comment)")
    ans(argument_tokens)
    ...
    while(true)
      ...
      if(is_expression
         AND NOT expression_end
         AND NOT expression_begin)
      ...
      if(expression_end)
        ...
        set(compiled_code
            "${compiled_code}${current_compiled_code}set(${ref} ${value})\n")
    ...
    if(current_arguments)
      cmake_token_range_replace("${args_begin};${args_end}"
                                "${current_arguments}")
    endif()
