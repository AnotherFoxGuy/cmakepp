macro(arguments_tokenize first last)
  ...
  if(NOT ${first} GREATER ${_last})
    ...
    # _message("input '${input}'") tokenize the string string(REPLACE "" ";;"
    # input "${input}")
    set(token_chars "{},:=&\\*\\$\\.\\-\\+\\|\\^%#@!\\?\\/~<>")
    ...
    if(NOT error)
      ...
      # use the index indicator  so that normal numbers are not confused with
      # token indices
      set(token_codes ${token_strings})
      string(REGEX REPLACE ";[0-9]+;" ";1;" token_codes "${token_codes}")
      string(REGEX REPLACE "[\"']([^\"'\\]|([\\][\"'])|([\\][\\])|([\\]))*[\"']"
                           "2" token_codes "${token_codes}")
      ...
      string(REGEX REPLACE "[^;${token_chars}]*" "32" token_codes
                           "${token_codes}")
