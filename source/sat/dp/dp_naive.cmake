...
function(dp_naive f)
    ...
    while(true)
        # decide which literal to try next to satisfy clauses returns true if decision was possible if no decision is made all clauses are satisfied and the algorithm terminates
        # with success
        dp_naive_decide()
        ans(decision)
        if(NOT decision)
        ...
        # propagate decision if a conflict occurs backtrack when backtracking is impossible the algorithm terminates with failure
        while(true)
...
# return false if no unassigned variables remain true otherwise adds new decision layer to decision stack
...
function(dp_naive_bcp)
    ...
    # message(FORMAT "propagating {context.f.literal_map.${decision}} = ${value} => deduced: ${result}") foreach(li ${result})
...
function(dp_naive_resolve_conflict)
    ...
    # undo decisions until a decision is found which was not tried the `other way` ie inversing the literals value
