...
# if(NOT target_namespace) log("no target namespace is specified - defaulting to package id '${package_id}'") string_to_target_name("${package_id}") ans(target_namespace) endif()

parameter_definition(cmake_import_target_from_build_info_targets <--target-namespace{"namespace for targets of package handle"}=>target_namespace:<target_identifier>>)
function(cmake_import_target_from_build_info_targets)
    ...
    # map_set(${cmake_target} include_dirs) map_set(${cmake_target} library) ## if library == empty -> use name as library name
    ...
    foreach(config ${configs})
        ...
        # map_set(${config_properties} imported_location ${libs}) map_set(${config_properties} imported_implib "") map_set(${config_properties} content_files ${output})
...
parameter_definition(cmake_target_import <--cmake-target{""}=>cmake_target:<map>>)
...
parameter_definition(build_info_targets_import <--target-namespace{"namespace for targets of package handle"}=>target_namespace:<target_identifier>>)
function(build_info_targets_import)
    ...
    # set(bin_dir bin) set(lib_dir lib)

    # set(include_dirs) set(implib) set(location) set(configs)

    # map_tryget(${build_parameters} install_dir) ans(install_dir)

    # set(include_dir include)

    # map_tryget("${build_parameters}" config) ans(cfg) string_toupper("${cfg}") ans(cfg) list(APPEND configs "${cfg}")

    # glob("${install_dir}/${bin_dir}/*${CMAKE_SHARED_MODULE_SUFFIX}") ans(shared_lib)

    # glob("${install_dir}/${lib_dir}/*${CMAKE_STATIC_LIBRARY_SUFFIX}") ans(static_lib)

    # set(include_dirs ${include_dirs} "$<$<CONFIG:${cfg}>:${install_dir}/${include_dir}>")

    # if(shared_lib) if(NOT location) map_new() ans(location) endif() list(LENGTH shared_lib nLibs) if("${nLibs}" GREATER 1) message(FATAL_ERROR "cannot handle shared nlibs greater
    # 1") endif() map_set("${location}" "${cfg}" "${shared_lib}") endif()

    # if(static_lib) if(NOT implib) map_new() ans(implib) endif()

    # list(LENGTH implib nLibs) if("${nLibs}" GREATER 1) message(FATAL_ERROR "cannot handle implib nlibs greater 1") endif() map_set("${implib}" "${cfg}" "${static_lib}") endif()

    # set(global GLOBAL) set(linkage SHARED ) # STATIC, INTERFACE

    # if(NOT location AND NOT implib) set(linkage INTERFACE) else()

    # endif()

    # if(location) set(linkage SHARED) elseif(location ) endif()

    # add_library(${target_name} ${linkage} IMPORTED ${global}) target_set(${target_name} INTERFACE_INCLUDE_DIRECTORIES ${include_dirs})

    # if(implib) map_keys(${implib}) ans(cfgs) foreach(cfg ${cfgs}) map_tryget(${implib} ${cfg}) ans(lib) target_set(${target_name} "IMPORTED_IMPLIB_${cfg}" "${lib}") endforeach()
    # endif()

    # if(location) map_keys(${location}) ans(cfgs) foreach(cfg ${cfgs}) map_tryget(${location} ${cfg}) ans(loc) target_set(${target_name} "IMPORTED_LOCATION_${cfg}" "${loc}")
    # endforeach() endif()
