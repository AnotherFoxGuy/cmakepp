...
function(build_info_evaluate_targets)
  ...
  foreach(build_info ${build_infos})
    ...
    foreach(evaluated_build_target ${evaluated_build_targets})
      ...
      map_append(${evaluated_targets} "${target_name}"
                 "${evaluated_build_target}")
    endforeach()
...
# if(NOT target_namespace) log("no target namespace is specified - defaulting to
# package id '${package_id}'") string_to_target_name("${package_id}")
# ans(target_namespace) endif()

parameter_definition(
  cmake_import_target_from_build_info_targets
...
function(cmake_import_target_from_build_info_targets)
  arguments_extract_defined_values(0 ${ARGC}
                                   cmake_import_target_from_build_info_targets)
  ...
  foreach(target_configuration ${target_configurations})
    map_set_default("${target_configuration}" cmake_target_name
                    "${cmake_target_name}")
  endforeach()
  ...
  # map_set(${cmake_target} include_dirs) map_set(${cmake_target} library) ## if
  # library == empty -> use name as library name
  ...
  foreach(config ${configs})
    ...
    if(NOT target_configuration)
      fatal(
        "could not associate config '${config}' with target_configuration config"
      )
    endif()
...
function(build_info_targets_import)
  ...
  foreach(package_target_name ${package_target_names})
    ...
    cmake_import_target_from_build_info_targets("${target_namespace}"
  ...
  # map_tryget("${build_parameters}" config) ans(cfg) string_toupper("${cfg}")
  # ans(cfg) list(APPEND configs "${cfg}")
  ...
  # "$<$<CONFIG:${cfg}>:${install_dir}/${include_dir}>")

  # if(shared_lib) if(NOT location) map_new() ans(location) endif() list(LENGTH
  # shared_lib nLibs) if("${nLibs}" GREATER 1) message(FATAL_ERROR "cannot
  # handle shared nlibs greater 1") endif() map_set("${location}" "${cfg}"
  # "${shared_lib}") endif()

  # if(static_lib) if(NOT implib) map_new() ans(implib) endif()

  # list(LENGTH implib nLibs) if("${nLibs}" GREATER 1) message(FATAL_ERROR
  # "cannot handle implib nlibs greater 1") endif() map_set("${implib}" "${cfg}"
  # "${static_lib}") endif()
  ...
  # if(implib) map_keys(${implib}) ans(cfgs) foreach(cfg ${cfgs})
  # map_tryget(${implib} ${cfg}) ans(lib) target_set(${target_name}
  # "IMPORTED_IMPLIB_${cfg}" "${lib}") endforeach() endif()

  # if(location) map_keys(${location}) ans(cfgs) foreach(cfg ${cfgs})
  # map_tryget(${location} ${cfg}) ans(loc) target_set(${target_name}
  # "IMPORTED_LOCATION_${cfg}" "${loc}") endforeach() endif()
