...
# created in the current directory *flags*: * *commands*: * `init` saves an
# initial cmake file at the current location * `target <target name> <target
# command> | "add" <target name>` target commands: * `add` adds the specified
# target to the end of the cmakelists file * `sources "append"|"set"|"remove"
# <glob expression>...` adds appends,sets, removes the source files specified by
# glob expressions to the specified target * `includes "append"|"set"|"remove"
# <path>....` adds the specified directories to the target_include_directories
# of the specified target * `links "append"|"set"|"remove" <target name>...`
# adds the specified target names to the target_link_libraries of the specified
# target * `type <target type>` sets the type of the specified target to the
# specified target type * `rename <target name>` renames the specified target
#
...
function(cmakelists_cli)
  ...
  elseif(NOT cmakelists)
    error("no CMakeLists.txt file found in current or parent directories"
          --function cmakelists_cli)
    return()
    ...
    elseif("${command}" STREQUAL "rename")
      ...
      if(NOT target)
        message(
          FATAL_ERROR
            FORMAT
            "no single target found for ${target_name} in {cmakelists.path}")
      endif()
