# `(<cmake token range> <predicate> [--skip <uint>] [--take <uint>]
# [--reverse])-><cmake invocation>...`
#
...
# access to the following variables: * invocation_identifier *
# invocation_arguments e.g. `invocation_identifier MATCHES "^add_.*$"` would
# return only invocations starting with add_ also see `eval_predicate`
# ~~~
# <cmake invocation> ::= {
...
function(cmake_invocation_filter_token_range range)
  ...
  while(take AND current)
    ...
    ans(line)
    cmake_token_range_filter("${invocation_token};${end}" type STREQUAL
                             "nesting" --take 1)
    ...
    cmake_token_range_filter_values(
      "${invocation_token};${arguments_after_end_token}" type MATCHES
      "(command_invocation)|(nesting)|(argument)")
    ans(invocation)
    ...
    # print_vars(invocation_token.type invocation_token.value predicate_holds
    # args)

    # check if invocation matches the custom predicate skip and take the
    ...
    if(predicate_holds)
      if(skip)
        ...
        map_capture_new(
          invocation_identifier invocation_arguments invocation_token
          arguments_begin_token arguments_end_token)
        ans_append(result)
        if(${take} GREATER 0)
