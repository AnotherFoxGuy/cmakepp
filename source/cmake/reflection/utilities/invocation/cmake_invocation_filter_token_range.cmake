...
function(cmake_invocation_filter_token_range range)
    ...
    while(take AND current)
        ...
        # print_vars(invocation_identifier invocation_arguments predicate_holds) print_vars(invocation_token.type invocation_token.value predicate_holds args)

        # check if invocation matches the custom predicate skip and take the specific invocations
        if(predicate_holds)
            if(skip)
                math(EXPR skip "${skip} - 1")
            else()
                cmake_token_advance(arguments_begin_token)
                map_capture_new(invocation_identifier invocation_arguments invocation_token arguments_begin_token arguments_end_token)
                ans_append(result)
                if(${take} GREATER 0)
