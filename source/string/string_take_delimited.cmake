...
# (__str_ref). Notes on the delimiter: - Default is double quote "" - Beginning
# and end delimiter can be specified - May only be a single char - Escaped
# delimiters are unescaped
#
# **Examples** set(in_ref_str "'a string'") string_take_delimited(in_ref_str ')
# # => in_ref_str equals "" and res equals "a string" set(in_ref_str "'a
# string'") string_take_delimited(in_ref_str "''") # => same as above
#
function(string_take_delimited __string_take_delimited_string_ref)
  ...
  string_take_regex(${__string_take_delimited_string_ref}
                    "${__string_take_delimited_regex}")
  ans(__string_take_delimited_match)
  if(NOT __string_take_delimited_match)
    return()
  endif()
  set("${__string_take_delimited_string_ref}"
      "${${__string_take_delimited_string_ref}}"
      PARENT_SCOPE)
...
function(string_take_delimited __str_ref)
  ...
  if("${input}" MATCHES "^${regex}")
    ...
    string(REPLACE "\\${delimiter_end}" "${delimiter_end}" res
                   "${CMAKE_MATCH_1}")
    set("${__str_ref}"
        "${input}"
        PARENT_SCOPE)
