...
function(template_compile input)
    ...
    string(REGEX REPLACE "${shorthand_indicator_code}(${regex_cmake_function})" "${delimiter_code}=\\1${delimiter_code}" input "${input}")

    string(
        REGEX
        REPLACE # "${shorthand_indicator_code}([^
                # ${shorthand_indicator_code}${delimiter_code}\"'\r\n\\/]+)"
                "${shorthand_indicator_code}((${regex_cmake_identifier})(\\.${regex_cmake_identifier})*)" "${delimiter_code}=\"{\\1}\"${delimiter_code}" input "${input}")
    ...
    foreach(fragment ${fragments})
        # message("${fragment}") decode brackets and semicolon in fragment now the fragment input is exactly the same as it was in input
