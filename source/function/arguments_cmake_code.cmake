  ...
  foreach(i RANGE ${__args_start} ${__args_end})
    ...
    elseif("${__args_current_token}_" STREQUAL ")_")
      ...
      if("${__args_current_token}" MATCHES "[ \\$\"\\(\\)#\\^\t\r\n\\\;]")
        # encoded list encode cmake string... @ todo this has to be extracted
        # however it does not work correctly because of macro evaluation when
        # using cmake_string_escape2
        string(REPLACE ";" "${semicolon_code}" __args_current_token
                       "${__args_current_token}")
        string(REPLACE "\\" "\\\\" __args_current_token
                       "${__args_current_token}")
        string(REGEX REPLACE "([ \"\\(\\)#\\^])" "\\\\\\1" __args_current_token
                             "${__args_current_token}")
        string(REPLACE "\t" "\\t" __args_current_token
                       "${__args_current_token}")
        string(REPLACE "\n" "\\n" __args_current_token
                       "${__args_current_token}")
        string(REPLACE "\r" "\\r" __args_current_token
                       "${__args_current_token}")
    ...
    if(__args_invocation_complete)
      string(REPLACE ";" "" __args_current_invocation
                     "${__args_current_invocation}")
