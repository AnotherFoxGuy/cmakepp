# checks to see if an assertion holds true. per default this halts the program
# if the assertion fails usage: assert(<assertion> [MESSAGE <string>]
# [MESSAGE_TYPE <FATAL_ERROR|STATUS|...>] [RESULT <ref>]) <assertion> :=
# <truth-expression>|[<STRING|NUMBER>EQUALS <list> <list>] <truth-expression> :=
# anything that can be checked in if(<truth-expression>) <list> :=
# <ref>|<value>|<value>;<value>;... if RESULT is set then the assertion will not
# cause the program to fail but return the true or false if ACCU is set result
# is treated as a list and if an assertion fails the failure message is added to
# the end of result examples
#
# assert("3" STREQUAL "3") => nothing happens assert("3" STREQUAL "b") =>
# FATAL_ERROR assertion failed: '"3" STREQUAL "b"' assert(EXISTS
# "none/existent/path") => FATAL_ERROR assertion failed 'EXISTS
# "none/existent/path"' assert(EQUALS a b) => FATAL_ERROR assertion failed ''
# assert(<assertion> MESSAGE "hello") => if assertion fails prints "hello"
# assert(<assertion> RESULT res) => sets result to false if assertion fails or
# to true if it holds assert(EQUALS "1;3;4;6;7" "1;3;4;6;7") => nothing happens
# lists are equal assert(EQUALS 1 2 3 4 1 2 3 4) =>nothing happens lists are
# equal (see list_equal) assert(EQUALS C<list> <list> COMPARATOR <comparator>
# )... todo todo: using the variable result as a boolean check fails because the
# name is used inside assert
...
function(assert)
  ...
  # if no message type is set set FATAL_ERROR so execution halts on failing
  # assertion
  ...
  elseif(_FILE_CONTAINS)
    if(NOT _MESSAGE)
      set(_MESSAGE
          "assertion failed: file '${_FILE_CONTAINS}' does not contain: ${_UNPARSED_ARGUMENTS}"
      )
    endif()
  ...
  elseif(_COUNT OR "_${_COUNT}" STREQUAL _0)
    ...
    if(NOT _MESSAGE)
      set(_MESSAGE
          "assertion failed: expected '${_COUNT}' elements got '${len}'")
    endif()
  ...
  elseif(_MAP_MATCHES)
    ...
    if(NOT _MESSAGE)
      ...
      ans(actual)
      set(_MESSAGE
          "assertion failed: match failed: expected: '${expected}' actual:'${actual}'"
      )
    endif()
  ...
  elseif(_CONTAINS OR _MISSING)
    if(NOT _MESSAGE)
      set(_MESSAGE
          "assertion failed: list does not contain '${_CONTAINS}' list:(${_UNPARSED_ARGUMENTS})"
      )
    endif()
  ...
  # print message if assertion failed, SILENT is not specified or message type
  # is FATAL_ERROR
  if(NOT result)
  ...
  # depending on wether to accumulate the results or not set result to a boolean
  # or append to result list
  if(_ACCU)
    set(lst ${_RESULT})
