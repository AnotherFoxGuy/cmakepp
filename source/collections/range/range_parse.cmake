# `(<~range...>)-><range>`
#
# parses a range string and normalizes it to have the following form: `<range>
# ::= <begin>":"<end>":"<increment>":"<begin inclusivity:<bool>>":"<end
# inclusivity:<bool>>":"<length>":"<reverse:<bool>> these `<range>`s can be used
# to generate a index list which can in turn be used to address lists.
#
...
function(range_parse)
  ...
  # if there is more than one range group recursively invoke range_parse
  ...
  # get begin and end_inclusivity chars results in begin_inclusivity and
  # end_inclusivity to be either "<" ">" or " "
  ...
  # transform "<" ">" and " " to a true or false value " " means default
  # inclusivity
  ...
  if((${end} LESS ${begin} AND ${increment} GREATER 0)
     OR (${end} GREATER ${begin} AND ${increment} LESS 0))
    return()
  endif()
  ...
  # if begin or end contains a sign operator put it in parentheses
  if("${begin}" MATCHES "[\\-\\+]")
    set(begin "(${begin})")
  endif()
  if("${end}" MATCHES "[\\-\\+]")
  ...
  # calculate length of range (number of elements that are spanned) depending on
  ...
  set(range
      "${begin}:${end}:${increment}:${begin_inclusivity}:${end_inclusivity}:${length}:${reverse}"
  )
