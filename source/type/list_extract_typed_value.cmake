...
# ~~~
# the formal definition is relatively hard to understand but using examples it
# should be easy
#
# ~~~
...
function(list_extract_typed_value __lst __letsv_def)
  regex_cmake()
  set(__letsv_regex
      "^([<\\[])(${regex_cmake_flag})(\\{(.*)\\})?(=>(${regex_cmake_identifier}))?(:(.*))?(\\]|>)$"
  )
  if("${__letsv_def}" MATCHES "${__letsv_regex}")
    ...
    # set(__letsv_comment ${CMAKE_MATCH_4}) set comment if outvar was detected
    if(ARGN)
    ...
    elseif("${__letsv_type}" MATCHES "<(${regex_cmake_identifier})(.*)>(.*)")
      # _message("${__letsv_type} : 0 ${CMAKE_MATCH_0} 1  ${CMAKE_MATCH_1} 2
      # ${CMAKE_MATCH_2} 3 ${CMAKE_MATCH_3}")
      ...
      message(
        FATAL_ERROR
          "invalid __letsv_type __letsv_def: '${__letsv_type}' (needs to be inside angular brackets)"
      )
    endif()
    ...
    if(NOT __letsv_optional AND NOT "${__letsv_value}_" STREQUAL "_")
      if(__letsv_type
         AND NOT "${__letsv_type}" MATCHES "^(any)|(string)$"
         AND COMMAND "t_${__letsv_type}")
        ...
        if(NOT __letsv_success)
          message(
            FATAL_ERROR
              "could not parse ${__letsv_type} from '${__letsv_value}'")
        endif()
    ...
    set(__ans
        ${__letsv_identifier} ${__letsv_value}
        PARENT_SCOPE)
    set(${__lst}
        ${${__lst}}
        PARENT_SCOPE)
